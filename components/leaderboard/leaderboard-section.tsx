...
export default function LeaderboardSection({
  ...
  // Fetch data from Django API using TanStack Query
  const { data: topEarnersData, isLoading: isLoadingEarners } = useTopEarners();
  const { data: topBugReportersData, isLoading: isLoadingReporters } = useTopBugReporters();
  const { data: topPRContributorsData, isLoading: isLoadingContributors } = useTopPRContributors();
  const { data: topReferralsData, isLoading: isLoadingReferrals } = useTopReferrals();
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <LeaderboardCard
            ...
            entries={topEarnersData || defaultTopEarners}
          />
          <LeaderboardCard
            title="Top Bug Reporters"
            icon="fas fa-bug"
            entries={topBugReportersData || defaultTopBugReporters}
          />
          <LeaderboardCard
            title="TOP BLT PRs - Nov"
            icon="fas fa-code-branch"
            entries={topPRContributorsData || defaultTopPRContributors}
          />
          <LeaderboardCard
            title="Top Referrals"
            icon="fas fa-user-plus"
            entries={topReferralsData || defaultTopReferrals}
          />
        </div>
      </div>
    </section>
